 ...
 * 
 * This is used to go from the last day of a month to next/previous.
 * 
 ...
 */
class EndOfMonthCalendar implements IReportingDayCalendar {
	 ...
	 * false it will return null.
	 */
	EndOfMonthCalendar(DateTime from, DateTime to, boolean forceLowerBound, boolean forceUpperBound) {
		this.from = from;
		this.to = to;
		if (to == null) {
	...
	public DateTime previous() {
		...
		if (current == null) {
			if (forceUpperBound) {
				if (!to.isEqual(to.dayOfMonth().withMaximumValue())) {				
					troncatedPeriod = true;
				}
				current = to;
				return current;
		...
		if (current.isBefore(from)) {
			...
			if (!from.isEqual(from.dayOfMonth().withMaximumValue()) && forceLowerBound
